...
const { sendSuccess, sendError } = require("../utils/httpResponses");
const createTrailCore = require("../core/createTrailCore");
const updateTrailCore = require("../core/updateTrailCore");
...
exports.createTrail = (req, res) => {
  try {
    // Regras de negócio movidas para o Core
    const newTrail = createTrailCore(trails, req.body);

    // Agora apenas salva no "banco"
    trails.push(newTrail);
...
exports.updateTrail = (req, res) => {
  try {
    ...
    // Regras de autorização + validações agora no Core
    const updatedTrail = updateTrailCore(trails, id, req.body);

    // Aplica atualização ao mock
    const index = trails.findIndex((t) => t.id === id);
    trails[index] = updatedTrail;

    return sendSuccess(res, 200, updatedTrail);
  } catch (err) {
    return sendError(res, 400, err.message);
  }
};
